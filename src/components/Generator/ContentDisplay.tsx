...
const ContentDisplay: React.FC<ContentDisplayProps> = ({ title, content, type, isLoading, format }) => {
  ...
  const renderResourceLinks = () => {
    ...
    lines.forEach(line => {
      if (line.startsWith('*')) {
        // Modified regex to capture both text and URL
        const linkMatch = line.match(/\*\s*\[(.*?)\]\((.*?)\)/);
        if (linkMatch) {
          const [, text, url] = linkMatch; // Capture text and URL
          links.push({ text: text.trim(), url }); // Use captured text
        }
      }
    });
          ...
          <div className="flex flex-wrap gap-2">
            {links.map((link, index) => (
              <a
                ...
                className="inline-flex items-center px-4 py-2 bg-indigo-100 text-indigo-700 rounded-lg hover:bg-indigo-200 transition-colors duration-200 flex-wrap overflow-hidden text-ellipsis break-words"
              >
                {link.text}
              </a>
            ))}
